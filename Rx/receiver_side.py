...
def rotate_nliphase(rx_symbol, tx_symbol):
    '''
    ...
    assert rx_symbol.shape[0] ==  tx_symbol.shape[0]

    for i in range(rx_symbol):
        rx_symbol = rx_symbol[i,:] / np.sqrt(np.mean(np.abs(rx_symbol[i,:]) ** 2))
        tx_symbol = tx_symbol [i,:]/ np.sqrt(np.mean(np.abs(tx_symbol[i,:]) ** 2))
        average_phase = np.angle(np.sum(rx_symbol[i,:] / tx_symbol[i,:]))
        rx_symbol[i,:] = rx_symbol[i,:] * np.exp(-1j * average_phase)
